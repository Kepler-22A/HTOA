...
<body>
    ...
    <div>
        ...
        <div class="layui-form-item" style="display: inline-block;margin-left: 20px">
            <i class="layui-icon" lay-submit style="font-size: 20px;" title="修改一下公告吧！">&#xe61f;</i>
...
<script>
    $("#class").hide()//隐藏
    $("#title").attr("value","<%=request.getAttribute("title")%>")
    layui.use(['form', 'layedit', 'laydate'], function(){
        ...
        // layedit.setContent(array,'sdfdsfdsf');
        $("#demo").val('<%=request.getAttribute("context")%>'); //先赋值
        layedit.build('demo');  //渲染 build
        var bbb = layedit.build('demo');


        //监听提交
        form.on('submit', function(data){
             var aa = layedit.getContent(bbb);//获取文本框的数据
            ...
            var url = '';
            if(clazz != null && "" != clazz){
                url = '${pageContext.request.contextPath}/message/updateNoticeOK/'+title+'/'+noticeType+'/'+clazz+'/'+empid+'/'+id;
            }else {
                url = '${pageContext.request.contextPath}/message/updateNoticeOK2/'+title+'/'+noticeType+'/'+empid+'/'+id;
            }
            $.ajax({
                type:'post',
                async:true,
